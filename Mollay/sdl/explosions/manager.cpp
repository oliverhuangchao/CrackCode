...
Manager::Manager() :
  ...
  screen( io.getScreen() ),
  orbSurface( io.loadAndSet(gdata.getXmlStr("orb/file"), true) ),
  ...
  while ( not done ) {
    ...
    if ( orbExploded ) {
      if ( static_cast<ExplodingSprite*>(orb)->chunkCount() == 0 ) {
        Vector2f position(orb->getPosition());
        delete orb;
        orb = new Sprite("orb",
          position,
          Vector2f(gdata.getXmlInt("orb/speed/x"), 
                   gdata.getXmlInt("orb/speed/y")),
          orbFrame);
        orbExploded = false;
      }
    }
