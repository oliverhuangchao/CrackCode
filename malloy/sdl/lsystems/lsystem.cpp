...
LSystem::LSystem(const string &filename, unsigned steps) :
  ...
  sentence( )
  {
    cout << "Loading L-System Spec for " + filename + " ..." << endl;
    loadSpecification();
    displaySystem();
    ...
    cout << "stepsSoFar: " << stepsSoFar << endl;

    cout << "Making an image ..." << endl;
    fillSprite();
}
...
// Function countDrawingSteps counts only the steps
// that involve drawing a line;
void LSystem::countDrawingSteps( ) {
  string letters = "FAB";
...
void LSystem::reset( ) {
  cout << "resetting ..." << endl;
  while ( not stateStack.empty() ) stateStack.pop();
...
void LSystem::loadSpecification() {
  ...
  cout << "The contents of xmlData: " << endl;
  std::map<string, string>::const_iterator dptr = xmlData.begin();
  while ( dptr != xmlData.end() ) {
    cout << dptr->first << ", " << dptr->second << endl;
    ++dptr;
  }
...
void LSystem::fillSprite() { 
  ...
  if ( stepCount < sentence.size() ) {
    stepCount += stepsAtaTime;
  }
  else {
    cout << "done ..." << endl;
    return;
  ...
  while (count < stepCount && instructionCount < sentence.size()) {
    switch ( sentence[instructionCount] ) {
      case '-' :  {
      ...
      case 'F' :  case 'A' : case 'B' : {
        ++count;
        cout << "drawing " << strokeWidth << " pixels ..." << endl;
        int x1 = state.x + 
            static_cast<int>(strokeLength * cos(state.facing*PI/180));
        int y1 = state.y + 
            static_cast<int>(strokeLength * sin(state.facing*PI/180));
      ...
      default : {
        ...
        throw string("Unhandled switch/case: ")+sentence[count];
      }
    }
    ++instructionCount;
  }
